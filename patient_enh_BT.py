...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb 15 14:24:20 2023
...
# %%=====================================================================
hmax = np.max(H1-H_0)
ut_1 = 100*(np.count_nonzero((H1-H_0)/hmax>0.5))/(Iy*Ix)
ut_2 = 100*(np.count_nonzero((H1-H_0)/hmax>0.75))/(Iy*Ix)

print(hmax, ut_1, ut_2)
...
# %%=======================================
p_g  = np.zeros((m, Iy, Ix, 7))
p_a  = np.zeros((m, Iy, Ix, 7))
p_b  = np.zeros((m, Iy, Ix, 7))
r_g  = np.random.rand(m, Iy, Ix)
r_a = np.random.rand(m, Iy, Ix)
...
label1 = ['untreated', 'S1', 'S2/3', r'$S^{new}$', 'S1+S2/3', r'S1+$S^{new}$', r'S2/3+$S^{new}$', r'S1+S2/3+$S^{new}$']
label2 = ['untreated', 'E1', 'E2', 'E3', 'E1+E2', 'E1+E3', 'E2+E3', 'E1+E2+E3']
left = np.arange(8)
ax1.bar(left, ave075, yerr=SD075, capsize=5, tick_label=label2)
ax1.axhline(y=1.0, linestyle='dashed', color='black')
ax1.set_ylim([0, 1.35])
fig1.savefig("patient_enh_BTp.png")

# %%=================================================================================
fig2 = plt.figure(figsize=(10,5))
ax2 = fig2.add_subplot(111)
ax2.bar(label2, ave05, width=-0.4, yerr=SD05, capsize=5, align='edge', label=">0.5")
ax2.bar(label2, ave075, width=0.4, yerr=SD075, capsize=5, align='edge', label=">0.75")
ax2.axhline(y=1.0, linestyle='dashed', color='black')
ax2.legend(loc="best", fontsize=10)
ax2.set_ylim([0, 1.35])
fig2.savefig("patient_enh_BT.png")
